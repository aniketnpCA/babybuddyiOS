...
final class NotesViewModel {
    ...
    func loadToday(childID: Int) async {
        ...
        do {
            let response: PaginatedResponse<Note> = try await APIClient.shared.get(
    ...
    func loadWeek(childID: Int) async {
        ...
        do {
            ...
            weekNotes = response.results.sorted { $0.time > $1.time }
        } catch {
            if (error as? URLError)?.code != .cancelled {
                self.error = error.localizedDescription
            }
    ...
    func loadCustomRange(childID: Int, start: Date, end: Date) async {
        ...
        do {
            ...
            customNotes = response.results.sorted { $0.time > $1.time }
        } catch {
            if (error as? URLError)?.code != .cancelled {
                self.error = error.localizedDescription
            }
    ...
    func deleteNote(id: Int) async throws {
        try await APIClient.shared.delete(path: APIEndpoints.note(id))
    }
}
